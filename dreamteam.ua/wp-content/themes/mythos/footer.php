...
<?php 
    if($myAdress) {
        foreach($myAdress as $adress) {
            echo '<p style="text-align: center; font-weight: 400; font-size: 23px;padding: 10px;">Welcome to  '.$adress->city.', '.$adress->adress.'</p>';
        }
    }
?>
...
<div id="bottom-wrap"  class="footer">
    ...
    <?php if ( get_theme_mod( 'footer_en', true )) { ?>
        <?php
            ...
            $myAdress = $wpdb->get_results("
                SELECT * FROM `oursite`.`wp_adress`
            ");
            if($myAdress) {
                foreach($myAdress as $adress) {
                    echo '<p style="text-align: center; font-weight: 400; font-size: 23px;padding: 10px;">Welcome to  '.$adress->city.', '.$adress->adress.'</p>';
